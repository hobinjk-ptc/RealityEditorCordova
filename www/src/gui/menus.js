...
realityEditor.gui.buttons.buttonActionEnter = function (event){
    ...
    button[event._button].bg.setAttribute("class", button[event._button].bg.classList[0]+" touched");
};

realityEditor.gui.buttons.buttonActionLeave = function (event){
    // make button react to touch
    var button = realityEditor.gui.menus.buttons;
    if(button[event._button].bg.classList[1] !=="active") {
        button[event._button].bg.setAttribute("class", button[event._button].bg.classList[0] + " " + "inactive");
    } else {
        button[event._button].bg.setAttribute("class", button[event._button].bg.classList[0] + " " + "active");
    }
};
...
/// send active user interface status in to the AR-UI
    ...
    // send to local interfaces too
    for (var localObjectKey in globalStates.localObjects) {
        if (globalStates.localObjects[localObjectKey].visible) {
            globalDOMCach["iframe" + localObjectKey].contentWindow.postMessage(JSON.stringify(msg), "*");
        }
    }
...
//console.log("Down on: "+event._button);
...
realityEditor.gui.menus.pointerUp = function(event) {
  //  console.log("Up on: "+event._button);

    realityEditor.gui.buttons.sendInterfaces(event._button);
   ...
   // console.log(realityEditor.gui.search.getVisibility());
    if(realityEditor.gui.search.getVisibility() && event._button !== "realitySearch"){
...
realityEditor.gui.menus.pointerEnter = function(event) {
   // console.log("Enter on: "+event._button);
...
realityEditor.gui.menus.pointerLeave = function(event) {
  //  console.log("Leave on: "+event._button);
...
realityEditor.gui.menus.pointerMove = function(event) {
   // console.log("Move on: "+event._button);
};
